...
#' @param time_var The column variable that designates the time of the data, e.g., it may be a 0 if data was collected on the first day of treatment and a 6 if data is collected six days later.
...
#'
#' @return A list of two data frames that are subsets of the original data frame, separated by their `time_var` status. The data frames will have the same size in rows.
#'
#' @details This function checks if the input data frame, ID, time variable, and column variables are valid. It ensures that the specified pre and post values exist within the time variable column. The function then filters the data for each time point, orders the IDs, and returns a list containing the filtered data frames.
#'
#' @examples
#' # Load example data
...
#' result <- filter_by_time_var(pre_post_data_example, id = "ID", time_var = "time", pre = 0, post = 12, vars = c("v1", "v2"))
#' print(result$pre)
#' print(result$post)
#'
#' @export
